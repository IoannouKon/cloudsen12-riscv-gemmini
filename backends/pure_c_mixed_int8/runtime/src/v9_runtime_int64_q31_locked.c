...
}else if(o->kind==OP_ADD){Tensor*bb=&T[o->b];if(!alloc8(z,o))return 9;size_t n=(size_t)o->n*o->c*o->h*o->w;int8_t*x=a->p,*y=bb->p,*q=z->p;for(size_t i=0;i<n;i++){int v=rq8(x[i],o->m1,o->s1)+rq8(y[i],o->m2,o->s2);q[i]=v>127?127:v<-127?-127:v;}
}else if(o->kind==OP_CONCAT){Tensor*bb=&T[o->b];if(!alloc8(z,o))return 10;int8_t*q=z->p,*x=a->p,*y=bb->p;size_t plane=(size_t)o->h*o->w;for(int c=0;c<a->c;c++)for(size_t i=0;i<plane;i++)q[(size_t)c*plane+i]=rq8(x[(size_t)c*plane+i],o->m1,o->s1);for(int c=0;c<bb->c;c++)for(size_t i=0;i<plane;i++)q[(size_t)(a->c+c)*plane+i]=rq8(y[(size_t)c*plane+i],o->m2,o->s2);}}
