...
res={'pytorch_fp32_512x512':{'images':len(py),'reference':True,'correct_vs_pytorch':len(py)*262144,'wrong_vs_pytorch':0,'agreement_vs_pytorch':1.0,'mean_seconds':statistics.mean(py),'median_seconds':statistics.median(py)},'opencv_onnx_cpp_fp32_512x512':base('cpp_onnx_fp32.csv'),'pure_c_fp32_512x512':base('pure_c_fp32.csv'),'pure_c_mixed_int8_256x256':{'images':len(ir),'correct_vs_pytorch':total-bad,'wrong_vs_pytorch':bad,'agreement_vs_pytorch':(total-bad)/total,'target_correct_pixels':target,'target_wrong_pixels':total-target,'target_pixel_accuracy':target/total,'mean_seconds':statistics.mean(its),'median_seconds':statistics.median(its),'maximum_peak_rss_mib':max(im)/1024},'pytorch_fp32_256x256_timing':{'images':len(py256),'mean_seconds':statistics.mean(py256),'median_seconds':statistics.median(py256)},'notes':['512x512 FP32 backends are directly comparable.','256x256 mixed INT8 is reported separately because dimensions differ.','PyTorch memory is not included because it runs in Windows Python/CUDA while Linux RSS measures WSL processes.']}
