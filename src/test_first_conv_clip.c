...
int main(void) {
    ...
    float *conv_out = (float *)malloc(output_count * sizeof(float));
    float *clip_out = (float *)malloc(output_count * sizeof(float));
    float *ref = (float *)malloc(output_count * sizeof(float));

    if (!input || !weight || !bias || !conv_out || !clip_out || !ref) {
    ...
    if (!read_float_file("experiments/operator_tests/first_clip/clip_ref.bin", ref, output_count)) {
        printf("Failed to read Clip reference\n");
        return 1;
    }
    ...
    clip_float(conv_out, clip_out, (int)output_count, 0.0f, 6.0f);
